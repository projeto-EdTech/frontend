...
import { 
  ...
  Table, TableRow, TableCell, WidthType, VerticalAlign, AlignmentType, BorderStyle 
} from "docx";
...
import { Lock, X, Zap, ArrowUpNarrowWide, ArrowDownNarrowWide } from "lucide-react";
...
const StatsUser: React.FC<StatsUserProps> = ({ stats, subjectPerformance, monthlyProgress, isPremiumUser = false }) => {
  
  const { theme } = useTheme();
  // Lê o tier do usuário a partir da sessão (assim como em Questoes_Gemini)
  const { data: session } = useSession();
  const userTier = session?.user?.tier ?? 'FREE';
  // Considera premium apenas quando o tier for "Simula PRO".
    ...
    if (reportType === 'completo' && !completeReport && !isLoadingReport) {
      ...
      const fetchCompleteReport = async () => {
        ...
        try {
          ...
          console.error(error);
          setReportError(error instanceof Error ? error.message : 'Houve um problema ao gerar seu relatório.');
        } finally {
  ...
  return (
    <TabsContent value="estatisticas" className="mt-6" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      ...
      <div className="flex justify-center mb-6 lg:mb-8">
        ...
        >
                ...
                <Lock 
                  className={`w-3.5 h-3.5 transition-all duration-300 ml-0.5
                    ${reportType === 'completo' // Isso não deve acontecer para free users, mas é seguro
                      ? 'text-white/70' 
                      : theme === 'dark' ? 'text-gray-400/70' : 'text-gray-600/70'
                    }`
                  }
                />
              )}
          ...
          <div className={`absolute -bottom-1 left-1/2 -translate-x-1/2 w-0 h-0.5 transition-all duration-300 rounded-full
  ...
  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className={`rounded-2xl p-8 shadow-[0_6px_24px_rgba(0,0,0,0.06)] relative overflow-hidden border
          ...
          >
            ...
            <h3 className="text-2xl font-bold text-gray-700 mb-6 relative z-10">Estatísticas Gerais</h3>
            <div className="grid grid-cols-2 gap-6 relative z-10">
              <div className={`text-center p-4 rounded-xl border hover:shadow-lg transition-shadow hover:scale-[1.02] transform hover:shadow-blue-500/30 hover:border-blue-500/20
                ${theme === 'dark' ? 'bg-[#2c2c2e]/70 border-white/10' : 'bg-white/90 border-black/10'}`}>
                <p className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'} mb-2`}>Simulados</p>
                <p className={`text-3xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-700'}`}>{stats.simulados}</p>
              </div>
              <div className={`text-center p-4 rounded-xl border hover:shadow-lg transition-shadow hover:scale-[1.02] transform hover:shadow-green-500/30 hover:border-green-500/20
                ${theme === 'dark' ? 'bg-[#2c2c2e]/70 border-white/10' : 'bg-white/90 border-black/10'}`}>
                <p className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'} mb-2`}>Questões</p>
                <p className={`text-3xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-700'}`}>{stats.questoes}</p>
              </div>
              <div className={`text-center p-4 rounded-xl border hover:shadow-lg transition-shadow hover:scale-[1.02] transform hover:shadow-green-600/30 hover:border-green-600/20
                ${theme === 'dark' ? 'bg-[#2c2c2e]/70 border-white/10' : 'bg-white/90 border-black/10'}`}>
                <p className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'} mb-2`}>Acertos</p>
                <p className={`text-3xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-700'}`}>{stats.acertos}</p>
              </div>
              <div className={`text-center p-4 rounded-xl border hover:shadow-lg transition-shadow hover:scale-[1.02] transform hover:shadow-purple-500/30 hover:border-purple-500/20
                ${theme === 'dark' ? 'bg-[#2c2c2e]/70 border-white/10' : 'bg-white/90 border-black/10'}`}>
                <p className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'} mb-2`}>% de Acertos</p>
                <p className={`text-3xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-700'}`}>{stats.percentagem}%</p>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 relative">
            ...
            {monthlyProgress.length > 0 ? (
              ...
              <div className={`flex items-center justify-center h-full text-center py-10 text-gray-500 rounded-2xl shadow-[inset_0_1px_0_rgba(255,255,255,0.6)] p-8 border
        ...
        <div className={`backdrop-blur-xl border rounded-3xl shadow-[0_10px_40px_rgba(0,0,0,0.08)] transition-all duration-500 relative overflow-hidden flex flex-col
          ${theme === 'dark' 
        ...
        `}>
          ...
          {/* ========== HEADER FIXO ========== */}
          <div className={`relative z-20 px-6 lg:px-8 py-5 border-b transition-all duration-300 rounded-t-3xl
            ${theme === 'dark' 
          ...
          `}>
            <div className="flex items-center justify-between flex-wrap gap-4">
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                ...
                {isLoadingReport && (
                  <div className={`flex items-center gap-2 px-4 py-2 rounded-xl backdrop-blur-md border animate-pulse
                    ${theme === 'dark' 
                      ? 'bg-green-500/10 border-green-400/30' 
                      : 'bg-green-500/10 border-green-400/30'}
                  `}>
                    <div className="w-2 h-2 bg-green-500 rounded-full animate-ping" />
                    <span className={`text-xs font-semibold ${theme === 'dark' ? 'text-green-300' : 'text-green-700'}`}>
                      Gerando...
                    </span>
                  </div>
                )}
          ...
          {isLoadingReport && !completeReport && (
            ...
            <div className="relative flex flex-col items-center justify-center min-h-[600px] px-8 py-12">
              ...
              <div className={`relative z-10 w-full max-w-2xl rounded-3xl p-10 backdrop-blur-xl border shadow-2xl transition-all duration-500
                ${theme === 'dark'
              ...
              `}>
                ...
                <div className="flex justify-center mb-8">
                  <div className={`inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full backdrop-blur-md border transition-all duration-300 animate-pulse
                    ${theme === 'dark'
                ...
                <div className="relative mb-8">
                  ...
                  <div className={`absolute -inset-1 rounded-3xl opacity-75 blur-lg transition-all duration-1000 animate-pulse
                    ${theme === 'dark'
                  ...
                  `}>
                    ...
                    >
                      <source src="/Mascote/Animações/Camaleão_Animado_Digitando_no_Notebook.mp4" type="video/mp4" />
                ...
                <div className="space-y-4 text-center mb-8">
                  ...
                  <p className={`text-base leading-relaxed max-w-lg mx-auto
                    ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}
                  `}>
                    Nossa inteligência artificial está analisando seu desempenho e preparando insights personalizados para potencializar seus estudos.
                  </p>
                </div>

                {/* Indicadores de processo */}
                <div className={`space-y-3 mb-8 p-5 rounded-2xl backdrop-blur-sm border
                  ${theme === 'dark'
                ...
                `}>
                  {[
                    ...
                    { label: 'Identificando padrões de aprendizado', delay: '0.5s' },
                    { label: 'Gerando recomendações personalizadas', delay: '1s' }
                  ].map((item, index) => (
                    <div key={index} className="flex items-center gap-3 animate-pulse" style={{ animationDelay: item.delay, animationDuration: '2s' }}>
                ...
                <div className="space-y-3">
                  ...
                  <div className={`relative h-2.5 rounded-full overflow-hidden shadow-inner
                ...
                `}>
                  <div className="flex items-start gap-3">
                    <svg className={`w-5 h-5 mt-0.5 flex-shrink-0 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <p className={`text-xs leading-relaxed
                      ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}
                    `}>
                      Este processo pode levar alguns instantes. Estamos utilizando inteligência artificial avançada para garantir a máxima precisão em sua análise.
                    </p>
                  </div>
                </div>
              </div>
          ...
          {reportError && (
            ...
            <div className="relative flex flex-col gap-6 justify-center items-center min-h-[500px] p-8 text-center">
              ...
              <div className="relative">
                ...
                {/* Círculo principal */}
                <div className={`relative flex items-center justify-center w-24 h-24 rounded-full border-4 transition-all duration-300
                  ${theme === 'dark' 
                ...
                `}>
                  <svg className={`w-14 h-14 ${theme === 'dark' ? 'text-red-400' : 'text-red-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="space-y-4 max-w-lg">
                <h3 className={`text-2xl font-bold ${theme === 'dark' ? 'text-red-400' : 'text-red-600'}`}>
                  Ops! Algo deu errado
                </h3>
          ...
          {completeReport && (
            ...
            <div className="relative flex flex-col flex-1 min-h-0">
              ...
              <div className="relative flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar">
                ...
                <article className={`p-8 lg:p-12 max-w-none transition-opacity duration-500 ${isLoadingReport ? 'opacity-90' : 'opacity-100'}`}>
                <ReactMarkdown 
            ...
            {/* ========== FOOTER FIXO ========== */}
            <div className={`relative z-20 px-6 lg:px-8 py-5 border-t transition-all duration-300 rounded-b-3xl
              ${theme === 'dark' 
                ...
                <div className="flex items-center gap-4 flex-1 min-w-0">
                  ...
                  <div className={`flex items-center gap-2 px-4 py-2 rounded-xl backdrop-blur-md border
                    ${theme === 'dark' 
                      ...
                      : 'bg-blue-500/5 border-blue-400/20'}
                  `}>
                    <svg className={`w-4 h-4 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      const reportParts = completeReport.split(/^-{3,}\s*$/m);
                      const coverMarkdown = reportParts[0] || '';
                      // CORREÇÃO: Rejunta o conteúdo principal com '---' (ou '\n---\n')
                      const mainReportMarkdown = reportParts.slice(1).join('\n---\n').trim();

                      const coverLines = coverMarkdown.split(/\r?\n/).filter(line => line.trim() !== '');
                      
                      // Valores padrão
                      let tituloRelatorio = 'Relatório de Desempenho Acadêmico';
                      ...
                      coverLines.forEach(line => {
                        const trimmedLine = line.replace(/^\*?\s*/, '').trim(); // Remove '*' e espaço
                        
                        if (trimmedLine.startsWith('Estudante:')) {
                          estudante = trimmedLine.substring('Estudante:'.length).trim();
                        } else if (trimmedLine.startsWith('Período Analisado:')) { // CORREÇÃO da chave
                          periodo = trimmedLine.substring('Período Analisado:'.length).trim();
                        } else if (trimmedLine.startsWith('Mentor Responsável:')) {
                          mentor = trimmedLine.substring('Mentor Responsável:'.length).trim();
                        } else if (trimmedLine.startsWith('Data de Geração:')) {
                          data = trimmedLine.substring('Data de Geração:'.length).trim();
                        } else if (
                          trimmedLine.startsWith('Tipo:') &&
                          tituloRelatorio === 'Relatório de Desempenho Acadêmico' // Pega o título
                        ) {
                           tituloRelatorio = trimmedLine.substring('Tipo:'.length).trim();
                        }
                      });
                      ...
                      const coverPageParagraphs = [
                        ...
                        new Paragraph({ text: "", style: "NormalABNT", spacing: { after: 1500 } }), // Espaço
                        
                        new Paragraph({
                          children: [new TextRun({ text: tituloRelatorio, bold: true, size: 32 })], // 16pt
                          style: "TituloABNT",
                          alignment: "center",
                        }),
                        
                        new Paragraph({ text: "", style: "NormalABNT", spacing: { after: 2500 } }), // Mais espaço
                        ...
                        new Paragraph({
                          children: [
                            new TextRun({ text: "Período de Análise: ", bold: true }),
                            new TextRun(periodo),
                          ],
                          style: "NormalABNT",
                          alignment: "left",
                        ...
                        new Paragraph({
                          children: [
                            new TextRun({ text: "Mentor Responsável: ", bold: true }),
                            new TextRun(mentor),
                          ],
                          style: "NormalABNT",
                          alignment: "left",
                        ...
                        new Paragraph({ text: "", style: "NormalABNT", spacing: { after: 3500 } }), 
                      ...
                      const parseBold = (text: string) => {
                        ...
                        return parts.map((part) =>
                          typeof part === 'string'
                            ? new TextRun(part)
                            : new TextRun({ text: part.text, bold: true })
                        );
                      };
                      ...
                      const mainContentParagraphs: (Paragraph | Table)[] = []; // O array agora aceita Parágrafos OU Tabelas
                      let i = 0;
                      ...
                      while (i < lines.length) {
                        ...
                        if (trimmed.startsWith('|') && lines[i + 1] && lines[i + 1].trim().startsWith('|:--')) {
                          ...
                          const alignments = parseTableLine(alignmentLine).map(a => {
                          ...
                          const headerCells = parseTableLine(headerLine).map((cell, index) => {
                            return new TableCell({
                              children: [new Paragraph({ 
                                children: [new TextRun({ text: cell, bold: true, color: "FFFFFF" })], // Texto em negrito e branco
                                style: "NormalABNT", 
                          ...
                          while (i < lines.length && lines[i].trim().startsWith('|')) {
                            const rowCells = parseTableLine(lines[i]).map((cell, index) => {
                              return new TableCell({
                                children: [new Paragraph({ 
                          ...
                          mainContentParagraphs.push(new Table({
                            rows: tableRows,
                            width: { size: 100, type: WidthType.PERCENTAGE }, // Tabela ocupa 100% da largura
                            borders: { // Bordas finas
                        ...
                        if (/^-{3,}$/.test(trimmed)) {
                          ...
                          mainContentParagraphs.push(new Paragraph({
                            children: parseBold(line.replace(/^>\s?/, "")),
                          ...
                          mainContentParagraphs.push(new Paragraph({
                            children: parseBold(trimmed.replace(/^\* /, "")),
                            style: "NormalABNT",
                            bullet: { level: 0 },
                          }));
                        } else if (/^  \* (.*)/.test(line)) {
                          ...
                          mainContentParagraphs.push(new Paragraph({ text: "", style: "NormalABNT" }));
                        } else {
                          ...
                          paragraphStyles: [
                            {
                              ...
                              paragraph: { spacing: { line: 360 }, alignment: "both" }, // 1.5 e justificado
                            },
                            {
                              id: "TituloABNT",
                              name: "Título ABNT",
                              ...
                              paragraph: { spacing: { after: 240, line: 360 }, alignment: "center" },
                            },
                            {
                              id: "SubtituloABNT",
                              name: "Subtítulo ABNT",
                              ...
                              paragraph: { spacing: { after: 120, line: 360 }, alignment: "left" },
                            },
                            {
                              id: "CitacaoLongaABNT",
                              ...
                              paragraph: { indent: { left: 2268 }, spacing: { line: 240 }, alignment: "both" }, // Recuo 4cm, espaço 1.0
                            },
                            {
                              id: "CitacaoCurtaABNT",
                              ...
                              next: "NormalABNT",
                              quickFormat: true,
                              run: { font: "Arial", size: 24, italics: true }, // 12pt
                              paragraph: { spacing: { line: 360 }, alignment: "both" },
                            },
                            {
                              id: "ReferenciaABNT",
                              ...
                              quickFormat: true,
                              run: { font: "Arial", size: 20 }, // 10pt
                              paragraph: { spacing: { line: 240 }, alignment: "both" }, // Espaço 1.0
                            },
                          ],
                        },
                        sections: [
                          {
                            ...
                            children: [
                              ...
                              new Paragraph({ text: "", style: "NormalABNT" }),
      ...
      <UpgradeModal isOpen={isUpgradeModalOpen} onClose={() => setIsUpgradeModalOpen(false)} theme={theme} />
    </TabsContent>
  );
};
