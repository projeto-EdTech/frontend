...
interface LibraryUniversityClientProps {
  universityInfo: University | null;
  slug: string;
}

export default function LibraryUniversityClient({
  universityInfo,
  slug,
}: LibraryUniversityClientProps) {
  ...
  const [selectedDay, setSelectedDay] = useState<number | null>(
    dayParam ? Number(dayParam) : null,
  );
  const [time, setTime] = useState<string>(timeParam || "");

  const error = !universityInfo ? "Universidade não encontrada" : null;
  ...
  const availableDays: number[] = useMemo(
    () =>
      selectedYear && universityInfo?.dia
        ? universityInfo.dia[selectedYear] || []
        : [],
  ...
  const syncQuery = useCallback(
    (next: {
      ...
      if (next.year !== undefined) {
        if (next.year !== null) newParams.set("year", String(next.year));
        else newParams.delete("year");
      }
      if (next.day !== undefined) {
        if (next.day !== null) newParams.set("day", String(next.day));
        else newParams.delete("day");
      }
      if (next.time !== undefined) {
        if (next.time) newParams.set("time", String(next.time));
        else newParams.delete("time");
      }

      // Only update if params actually changed
      if (currentParams.toString() !== newParams.toString()) {
        router.replace(`/library/${slug}?${newParams.toString()}`, {
          scroll: false,
        });
      }
    },
  ...
  const isFormValid = () =>
    selectedYear !== null && selectedDay !== null && isValidTime();
  ...
  const handleStartExam = () => {
    ...
    router.push(
      `/simulation/${slug}?year=${selectedYear}&day=${selectedDay}&time=${time}`,
    );
  };

  if (isNavigating) {
    return <LoadingScreen message="Iniciando o simulado..." />;
  }

  if (error || !universityInfo) {
    return (
      <div className="flex-1 relative z-10 flex items-center justify-center p-10">
        <div className="text-center bg-white/80 backdrop-blur-2xl backdrop-saturate-150 rounded-2xl p-10 shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-[#FF3B30]/20 max-w-md w-full">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
                ...
                <div className="order-2 lg:order-1">
                  <div className="mb-10 relative">
                    ...
                    <h2 className="text-[28px] font-semibold text-[#1d1d1f] mb-2 tracking-tight">
                      Configurar Simulado
                    </h2>
                  ...
                  <div className="space-y-7">
                      ...
                      <div className="flex flex-wrap gap-2.5">
                        ...
                        {selectedYear && availableDays.length === 0 && (
                          <div className="w-full text-center py-4 bg-[#FFF9E6] rounded-[10px] border border-[#FFD60A]/30">
                            <span className="text-[13px] text-[#8B7000]">
                              Nenhum dia configurado para este ano.
                            </span>
                          </div>
                        )}
                      </div>
                    ...
                    <div className="flex justify-center space-x-2 pt-1">
                      <div
                        className={`w-1.5 h-1.5 rounded-full transition-colors duration-300 ${selectedYear ? "bg-[#007AFF]" : "bg-[#d2d2d7]"}`}
                      ></div>
                      <div
                        className={`w-1.5 h-1.5 rounded-full transition-colors duration-300 ${selectedDay ? "bg-[#5856D6]" : "bg-[#d2d2d7]"}`}
                      ></div>
                      <div
                        className={`w-1.5 h-1.5 rounded-full transition-colors duration-300 ${isValidTime() ? "bg-[#34C759]" : "bg-[#d2d2d7]"}`}
                      ></div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="text-center">
                    <h1 className="text-[32px] md:text-[36px] font-semibold text-[#1d1d1f] mb-8 tracking-tight leading-tight">
                      {universityInfo.name?.toUpperCase() ||
                        universityInfo.slug?.toUpperCase() ||
                        "UNIVERSIDADE"}
                    </h1>
                    ...
                    <div className="w-40 h-40 mx-auto mb-8 relative group">
                      ...
                      <div className="relative w-full h-full !bg-white rounded-full shadow-[0_8px_30px_rgba(0,0,0,0.08)] flex items-center justify-center p-6 group-hover:shadow-[0_12px_40px_rgba(0,0,0,0.12)] transition-shadow duration-500 border border-gray-200">
                        <Image
                          src={
                            universityInfo.logo
                              ? universityInfo.logo.startsWith("/") ||
                                universityInfo.logo.startsWith("http")
                                ? universityInfo.logo
                                : `/${universityInfo.logo}`
                              : "/placeholder.svg"
                          }
                          alt={universityInfo.name || "University Logo"}
                          width={120}
                          height={120}
                    ...
                    <p className="text-[#6e6e73] text-[16px] font-normal mb-6 leading-relaxed">
                      ...
                      <div className="bg-gradient-to-br from-[#34C759]/8 to-[#30D158]/8 rounded-[14px] p-4 flex items-center space-x-3 border border-[#34C759]/10">
                        ...
                        <div className="text-left">
                          ...
                          <span className="text-[12px] text-gray-500 leading-relaxed">
                            Configure seu simulado e prepare-se para o sucesso!
                          </span>
                        </div>
                      </div>
                    </div>
